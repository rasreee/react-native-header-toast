...
import {NotificationPropOptions, NotificationProps} from './types';

export const HeaderToastProvider: React.FC = ({children}) => {
  const [current, setNotification] = useState<NotificationProps | undefined>();
  const [animatedValue] = useState(new Animated.Value(0));
  const hideTimer = useRef<number | undefined>();

  const handleSetProps = useCallback((props: NotificationPropOptions) => {
    const {
      title,
      textColor = 'black',
  ...
  const handleShow = useCallback(
    (props: NotificationPropOptions) => {
      if (!current) {
        handleSetProps(props);
        const shouldStartAnimation =
          props.autoClose === undefined ? true : props.autoClose;
